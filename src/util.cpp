...
const char * const BITCOIN_CONF_FILENAME = "uramacoin.conf";
const char * const BITCOIN_PID_FILENAME = "uramacoin.pid";
...
#else
    const char* pszModule = "uramacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Uramacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Uramacoin";
#else
    // Unix
    return pathRet / ".uramacoin";
#endif
#endif
}
